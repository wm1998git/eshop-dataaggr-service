package com.wm.eshop.dataaggr.rabbitmq;
 ...
 * 聚合数据变更的接收者<br/>
 * 1、接收到数据变更消息<br/>
 * 2、按照维度从redis中获取数据，聚合成一个维度：分类、品牌、基本信息、属性、规格、介绍的数据<br/>
 * 3、写入redis中维度聚合数据
 */
@Component
// 监听数据同步微服务：eshop-sync-service的RabbitMQ消费者向队列:aggr-data-change-queue发送的消息（json串）
...
public class AggrDataChangeQueueReceiver {
	...
	public void process(String message) {
		...
		if ("category".equals(dimType)) {
			processCategoryDimDataChange(messageJSONObject);
		} else if ("brand".equals(dimType)) {
	...
	private void processCategoryDimDataChange(JSONObject messageJSONObject) {
		...
		// 感觉上是多此一举：本应直接从队列：aggr-data-change-queue获取到数据后原样写入redis就完了
		// 但实际上是这样子的，我们这里是简化了数据结构和业务，实际上任何一个维度数据都不可能只有一个原子数据，肯定是结构多变的，结构比较复杂（有很多不同的表，即对应很多不同的原子数据）
		// 【实际上这里肯定是要将一个品牌对应的多个原子数据都从redis查询出来，然后聚合之后写入redis】
		Jedis jedis = jedisPool.getResource();
		String dataJSON = jedis.get("category_" + id);// 当初是由数据同步微服务写入到Redis里的
	...
	private void processProductDimDataChange(JSONObject messageJSONObject) {
		...
		if (productDataJSON != null && !"".equals(productDataJSON)) {
			...
			// 将商品属性对象信息设置到商品对象里
			String productPropertyDataJSON = jedis.get("product_property_" + id);// get第2次
			...
			// 将商品规格对象信息设置到商品对象里
			String productSpecificationDataJSON = jedis.get("product_specification_" + id);// get第3次
			...
			jedis.set("dim_product_" + id, productDataJSONObject.toJSONString());// 将聚合商品属性和规格之后的对象json串写入到redis
		} else {
